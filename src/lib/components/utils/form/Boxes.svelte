...
<div class="form-boxes" class:disabled={areDisabled}>
    ...
    <div class="boxes-container">
        {#each boxes as { text, value, isChecked, isDisabled }}
            <div class="form-box">
                <input id={value.toString()} {type} {name} {value} checked={isChecked} disabled={isDisabled || areDisabled}>
...
label::before {
    ...
    outline-offset: 5px;
    transition: background-color, outline-color, var(--field-transition-duration) ease-in-out;
    width: 25px;
}
